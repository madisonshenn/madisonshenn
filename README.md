- # 👋 Hello, I'm Madison!
- ## 🚀 Data professional skilled in statistical modeling, machine learning, and analytics, with cross-industry experience driving business decisions in marketing, finance, and venture capital.
- 📍 Pittsburgh, PA | Greater Los Angeles, CA 
- 📧 Email: [madisonshenn@gmail.com](mailto:madisonshenn@gmail.com)  
- 🔗 LinkedIn: [Connect Me On LinkedIn](https://www.linkedin.com/in/madison-s-0a2523198/)  
- 🐙 GitHub: [My Github](https://github.com/madisonshenn)

## About Me 👨‍💻
## Professional Highlights 🌟

- **Consulting Intern** – Oliver Wyman | Shanghai, China (2024)  
- **Data Analyst Intern** – Autohome Inc | Beijing, China (2023)  
- **Financial Service Office Consulting Intern** – Ernst & Young | Beijing, China (2023)  
- **Business Analyst Intern** – Credera, | GA, USA (2022)  
- **Venture Capital Intern** – HP Tech Venture | CA, USA (2022)
- **Human Resources Intern** – Bureau Executive | Shanghai, China and Malaysia (2020)

## Tech Toolbox 🛠️

**Languages:** Python, Java, HTML/CSS, MATLAB

**Statistical Analysis:** R, Stata, MS Excel (for data cleaning, analysis, and visualization)

**Relational Databases:** MySQL, PostgreSQL, Oracle SQL  
**NoSQL Databases:** MongoDB, Cassandra, Redis, Neo4j  
**Big Data Frameworks:** Apache Hadoop, Apache Spark

**ML & Statistical Libraries:** Scikit-learn, Keras, TensorFlow, Statsmodels, PyTorch

**Cloud & DevOps Tools:**  
- AWS: Glue, Redshift, QuickSight  
- Docker, Git

**Data Visualization:** Tableau, PowerBI

**Development Tools & Practices:**  
- Android Studio, IntelliJ IDEA, VS Code  
- Git, GitHub
- Unit testing & test frameworks: JUnit, PyTest, Mockito  

**Architectural Patterns & Design:**  
- RESTful API Design, MVC
- API documentation with Swagger/OpenAPI  
- Containerization with Docker  
- Build tools: Maven, Gradle  

## Education 🎓

- **Master of Science in Information Systems**, Carnegie Mellon University
- **Bachelor of Science in Statistics, Economics, and Data Science**, University of North Carolina Chapel Hill

## Certifications 📜

- **AWS**: [AWS Certified Cloud Practitoner](https://drive.google.com/file/d/1IpB4f0M6u4xevkRU9ximXiXKeiX7vX8F/view?usp=sharing)
- **IBM**: [Data Engineering](https://coursera.org/share/2d51e547afdaff3bfe75ba5c925ea2ce)
- **LinkedIn Learning**: Deep Learning, Machine Learning

### Interest Outside of School and Work:
- 👀 I’m interested in nature & travel, running, playing tennis, cooking, exploring authentic international restaurants around my city!!
- ⚡ Fun fact: I've visited 45+ countries and have a lot left on my wishlist!

> Reach out for job opportunities, collaborations, or simply to chat!!

<!---
madisonshenn/madisonshenn is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
